...
OSPF协议是一个内部网关协议（外部网关协议主要是用BGP）。其在路径选择上选择了Dijkstra最短路径算法SPF，其具有如下几个特点：

（1）向本自治系统中的所有路由器发送信息，使用“洪泛法”（即路由器向其所有相邻的路由器发送信息，而每个相邻路由器又将此信息发往其所有的相邻路由器，但不再发送给刚刚发来信息的那个路由器）。

（2）发送的信息是与本路由器相邻的所有路由器的链路状态，包括链路的费用、距离、时延等，相比RIP发送的是“到所有网络的距离和下一跳路由器”。

（3）只当链路状态发生变化时，路由器才向所有路由器洪泛发送此信息。而不像RIP那样，不管有没有发生变化，都定期交换路由信息。

（4）OSPF用的不是UDP，而是直接使用IP数据报。
